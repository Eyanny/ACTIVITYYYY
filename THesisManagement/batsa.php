<?php 
    ...
    if($con->connect_error){
        ...
        $stmt = $con->prepare("INSERT INTO data(thesis_Title, author_Name, department_Name, date_Published, indicator, abstract) 
        VALUES(?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssss", $thesis_Title, $author_Name, $department_Name, $date_Published, $indicators, $abstract);
        $stmt->execute();
        $stmt->close();

        // Retrieve the ID of the newly inserted thesis
